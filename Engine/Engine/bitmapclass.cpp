...
bool BitmapClass::Render(ID3D11DeviceContext* deviceContext, RECT position)
{
	bool result;


	// Re-build the dynamic vertex buffer for rendering to possibly a different location on the screen.
	result = UpdateBuffers(deviceContext, position);
	if (!result)
	{
		return false;
	}
...
bool BitmapClass::UpdateBuffers(ID3D11DeviceContext* deviceContext, RECT position)
{
...
bool BitmapClass::LoadTexture(ID3D11Device* device, WCHAR* filename)
{
	...
	m_Texture = new TextureClass(device);
	if (!m_Texture)
	{
		return false;
	}
