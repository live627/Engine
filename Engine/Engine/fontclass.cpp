...
void FontClass::BuildVertexArray(void* vertices, char* sentence, float drawX, float drawY)
{
	...
	for (i = 0; i < numLetters; i++)
	{
		...
		else
		{
			...
			vertexPtr[index].position = D3DXVECTOR3((drawX + m_Font[letter].size), (drawY - 64), 0.0f);  // Bottom right.
			vertexPtr[index].texture = D3DXVECTOR2(m_Font[letter].right, 1.0f);
			index++;

			vertexPtr[index].position = D3DXVECTOR3(drawX, (drawY - 64), 0.0f);  // Bottom left.
			...
			drawX = drawX + m_Font[letter].size + 4.0f;
		}
	}
